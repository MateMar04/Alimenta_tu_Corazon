...
WHERE NP.NOMBRE = 'Extrema Pobreza'
  AND B.EDAD < 15;

SELECT AB.ID_BENEFICIADO, B.NOMBRE
FROM BENEFICIADOS AS B
         INNER JOIN ASISNTECIAS_BENEFICIADOS AB on B.ID = AB.ID_BENEFICIADO
GROUP BY AB.ID_BENEFICIADO; /* BENEFICIADOS QUE ASISTIERON AL MENOS UNA VEZ AL MES */


SELECT AB.ID_BENEFICIADO, B.NOMBRE, COUNT(1) AS ASISTENCIAS
FROM BENEFICIADOS AS B
         INNER JOIN ASISNTECIAS_BENEFICIADOS AB on B.ID = AB.ID_BENEFICIADO
/* WHERE (ASISTENCIAS * 100) / 5 */
GROUP BY  AB.ID_BENEFICIADO; /* BENEFICIADOS QUE TUVIERON MAS DE UN 60% DE ASISTENCIA */
